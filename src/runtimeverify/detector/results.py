...
class DetectorExplanation(BaseModel):
    ...
    evidence: Dict[str, Any] = Field(
        default_factory=dict, description="Observed data points, rare transitions, or parameters acting as evidence"
    )
...
class DetectorResult(BaseModel):
    ...
    deviation_score: float = Field(..., description="Numerical metric indicating the degree of deviation")
    confidence: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Statistical confidence score or power (from 0.0 to 1.0)"
    )
    ...
    explanation: DetectorExplanation = Field(..., description="Detailed explainability payload backing the decision")
    raw_metrics: Dict[str, Any] = Field(
        default_factory=dict, description="Detector-specific raw statistics for debugging or metrics collection"
    )
