...
class ExecutionContext(BaseModel):
    ...
    environment: str = Field("development", description="Deployment tier (development, staging, production, sandbox)")
    agent_type: Optional[str] = Field(None, description="Archetype or role of the agent")
    ...
    tags: Dict[str, str] = Field(default_factory=dict, description="Operational key-value tags")
...
class Decision(BaseModel):
    ...
    status: str = Field(
        ..., description="Mitigation status action determined by policies (e.g., ALLOW, ALERT, BLOCK, PAUSE)"
    )
    ...
    detector_results: Dict[str, Any] = Field(
        default_factory=dict, description="Raw outputs from all executed detectors"
    )
