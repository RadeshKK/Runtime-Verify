...
class TelemetryContext(BaseModel):
    ...
    trace_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Trace identifier for execution grouping")
    parent_event_id: Optional[str] = Field(default=None, description="Identifier of the calling/parent event")
    correlation_id: Optional[str] = Field(default=None, description="Correlation identifier for external messaging systems")
