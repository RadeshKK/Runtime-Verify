...
from runtimeverify.events import FilesystemEvent, ToolEvent, LLMEvent
...
from runtimeverify.evaluation.baselines import RandomDetector, ThresholdDetector, FrequencyDetector
...
def test_alternative_models():
    ...
    # Adversarial
    adversarial = AdversarialAlternativeModel(
        vocabulary_size=8, high_risk_identifiers={"EXECUTE_COMMAND"}, high_risk_prob=0.8
    )
    assert (
        adversarial.get_probability(
            "ANY",
            ExecutionState(
    ...
    assert adversarial.get_probability(
        "ANY",
        ExecutionState(
            name="OTHER",
            category=StateCategory.TOOL,
            hierarchy=StateHierarchy(path=["TOOL", "OTHER"]),
            context=StateContext(session_id="s1", agent_id="a1"),
        ),
    ) == pytest.approx(0.2 / 7)
    ...
    assert (
        empirical.get_probability(
            "STATE",
            ExecutionState(
                name="NEXT",
                category=StateCategory.TOOL,
                hierarchy=StateHierarchy(path=["TOOL", "NEXT"]),
                context=StateContext(session_id="s1", agent_id="a1"),
            ),
        )
        == 0.5
    ...
    assert (
        empirical.get_probability(
            ...
            ExecutionState(
                name="ANY",
                category=StateCategory.TOOL,
                hierarchy=StateHierarchy(path=["TOOL", "ANY"]),
                context=StateContext(session_id="s1", agent_id="a1"),
            ),
        )
        == 0.125
...
def test_evaluation_replay():
    ...
    encoder = StateEncoderPipeline(
        DefaultTelemetryNormalizer(), DefaultResourceClassifier(), DefaultContextEnricher(), DefaultRuleEngine([])
    )
...
def test_evaluation_benchmark_and_reports():
    ...
    summary = ReportGenerator.generate_markdown(
        res["name"], res["metrics"], res["average_delay"], res["all_step_latencies"]
    )
...
def test_markov_predictor():
    ...
    matrix.probabilities = {"START": {"READ": 0.8, "WRITE": 0.2}, "READ": {"READ": 0.3, "WRITE": 0.7}}
...
def test_api_extended():
    ...
    # Train route
    train_payload = {"traces": [["START", "READ", "WRITE", "END"]], "smoothing": 0.05}
