    ...
    def observe(self, state: StateInterface) -> DetectorResult:
        ...
        return DetectorResult(
            ...
            explanation=DetectorExplanation(
                detector_name=self._name, summary="Normal execution flow matched", evidence={"value": state.name}
            ),
        )
...
def test_runtime_pipeline_and_engine():
    ...
    pipeline = RuntimePipeline(
        encoder=encoder, dispatcher=dispatcher, policy_engine=policy_engine, exporters=[mock_exporter]
    )
